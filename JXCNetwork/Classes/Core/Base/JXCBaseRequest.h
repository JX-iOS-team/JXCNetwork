...
FOUNDATION_EXPORT NSString *const JXCRequestValidationErrorDomain;

NS_ENUM(NSInteger) {
    JXCRequestValidationErrorInvalidStatusCode = -8,
    JXCRequestValidationErrorInvalidJSONFormat = -9,
};

///  HTTP Request method.
typedef NS_ENUM(NSInteger, JXCRequestMethod) {
    JXCRequestMethodGET = 0,
    JXCRequestMethodPOST,
    JXCRequestMethodHEAD,
    JXCRequestMethodPUT,
    JXCRequestMethodDELETE,
    JXCRequestMethodPATCH,
};

///  Request serializer type.
typedef NS_ENUM(NSInteger, JXCRequestSerializerType) {
    JXCRequestSerializerTypeHTTP = 0,
    JXCRequestSerializerTypeJSON,
};

///  Response serializer type, which determines response serialization process and
///  the type of `responseObject`.
typedef NS_ENUM(NSInteger, JXCResponseSerializerType) {
    /// NSData type
    JXCResponseSerializerTypeHTTP,
    /// JSON object type
    JXCResponseSerializerTypeJSON,
    /// NSXMLParser type
    JXCResponseSerializerTypeXMLParser,
};

///  Request priority
typedef NS_ENUM(NSInteger, JXCRequestPriority) {
    JXCRequestPriorityLow = -4L,
    JXCRequestPriorityDefault = 0,
    JXCRequestPriorityHigh = 4,
};
...
typedef void(^JXCRequestCompletionBlock)(__kindof JXCBaseRequest *request);

///  The JXCRequestDelegate protocol defines several optional methods you can use
///  to receive network-related messages. All the delegate methods will be called
///  on the main queue.
@protocol JXCRequestDelegate <NSObject>

@optional
///  Tell the delegate that the request has finished successfully.
///
///  @param request The corresponding request.
- (void)requestFinished:(__kindof JXCBaseRequest *)request;

///  Tell the delegate that the request has failed.
///
///  @param request The corresponding request.
- (void)requestFailed:(__kindof JXCBaseRequest *)request;
...
@interface JXCBaseRequest : NSObject
...
///  `JXCResponseSerializerType`. Note this value can be nil if request failed.
///
...
///  If you use `JXCResponseSerializerTypeJSON`, this is a convenience (and sematic) getter
...
///  Default is nil.
@property (nonatomic, weak, nullable) id<JXCRequestDelegate> delegate;
...
@property (nonatomic, copy, nullable) JXCRequestCompletionBlock successCompletionBlock;

///  The failure callback. Note if this value is not nil and `requestFailed` delegate method is
///  also implemented, both will be executed but delegate method is first called. This block
///  will be called on the main queue.
@property (nonatomic, copy, nullable) JXCRequestCompletionBlock failureCompletionBlock;

///  This can be used to add several accossories object. Note if you use `addAccessory` to add acceesory
///  this array will be automatically created. Default is nil.
//@property (nonatomic, strong, nullable) NSMutableArray<id<JXCRequestAccessory>> *requestAccessories;
...
///  The priority of the request. Effective only on iOS 8+. Default is `JXCRequestPriorityDefault`.
@property (nonatomic) JXCRequestPriority requestPriority;

///  Set completion callbacks
- (void)setCompletionBlockWithSuccess:(nullable JXCRequestCompletionBlock)success
                              failure:(nullable JXCRequestCompletionBlock)failure;
...
- (void)startWithCompletionBlockWithSuccess:(nullable JXCRequestCompletionBlock)success
                                    failure:(nullable JXCRequestCompletionBlock)failure;
...
///  HTTP request method.
- (JXCRequestMethod)requestMethod;

///  Request serializer type.
- (JXCRequestSerializerType)requestSerializerType;

///  Response serializer type. See also `responseObject`.
- (JXCResponseSerializerType)responseSerializerType;
